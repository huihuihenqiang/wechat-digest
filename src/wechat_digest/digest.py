...
class DigestGenerator:
    ...
    def generate(self, target_date: date, messages: list[StoredMessage]) -> str:
        ...
        prompt_messages = build_digest_prompt(target_date, messages[-self.max_input_messages :], self.timezone_name)
...
def build_digest_prompt(
    ...
    user_prompt = textwrap.dedent(
        f"""
        请基于以下微信群消息，为 {target_date.isoformat()} 生成中文 Markdown 日报。
        ...
        5. 每条重要结论都要能追溯到“群名 / 发送者”，不要写无法从原文推出的判断。

        输出格式：
        # 微信群日报 {target_date.isoformat()}
        ## 今日摘要
        - 3-6 条总览，先写最重要的变化、机会、风险和待办。
        ## 关键信息
        - 用“群名 / 发送者：事项”的形式列出重要信息；同类消息合并。
        ## 机会与资源
        ...
        ## 重要原话
        - 只摘录最关键的短句，每条不超过 40 字，并标明群名和发送者。
        ## 被忽略内容
...
def fallback_digest(target_date: date, messages: list[StoredMessage], max_items: int = 30) -> str:
    ...
    for message in messages[:max_items]:
        lines.append(f"- {message.group_name} / {message.sender}：{message.content}")
...
def _format_message_line(message: StoredMessage, timezone_name: str) -> str:
    ...
    return f"[{local_time}] {message.group_name} / {message.sender}: {message.content}"
